...
2026-08-13 [Session B]: CHILD PROFILES ARE NOW CROSS-DEVICE. New db/childProfiles.ts + app/actions/childProfiles.ts: profiles live in child_profiles, the client store keeps localStorage only as a cache. Every create/update/delete pushes, /profils + /compte pull via syncProfiles() (server wins; local-only profiles upload first so nothing is lost). NO MIGRATION: the client-only fields (avatar, language, streak, maxDuration, exact age, ageSetAt) ride in the existing appearance jsonb. Temp logins (user/user2/admin) get a deterministic uuid from the username + an auto-created users row, so they sync like real accounts. Also: new "doudou" character type (reuses the animal appearance branch, no age question).
2026-08-16 [Session A]: BATCH 3 (5-6 ans, 8 stories) -> 36/60. EVERY GENRE NOW HAS STORIES (metier was the last at 0). Batch 3 uses 16 paragraphs, not 12: a 12-paragraph draft came out ~400 words, under the 500 floor for this age, and 16 also yields 4 audio chapters so the player chapter controls switch on. New scratchpad/normalize-minutes.js derives readingMinutes from real FR word count per age band (<age> <wpm> <lo> <hi>); ran at 100 wpm for 5-6, which corrected 3 older entries. INTERACTIVE IS BLOCKED: interactiveTree() ignores its slug and returns ONE shared gamebook, so the two stories the plan marked [I] shipped as regular stories rather than serving the wrong plot; needs a per-slug registry + a tree written per story before any more are flagged.
2026-08-13 [Session A]: BATCH 2 of the library expansion: 7 new 3-4yo stories (28 total, target 60), FR+EN body/quiz/glossary + generated covers (manifest now 28). readingMinutes across the whole 3-4 range re-derived from word count (<280 = 3 min, <=340 = 4, else 5), which corrected 4 older entries that had drifted. Reusable inserter now lives at scratchpad/insert-batch.js: `node insert-batch.js <data> <paras> <minWords> <maxWords> <quiz>` validates paragraph count, word range, quiz shape, glossary words present in the body, styled dialogue and em dashes BEFORE writing. Batch 3 (5-6 ans, 8 stories) is next and is the first to use genre "metier", still at 0.
2026-08-12 [Session A]: Story page round. AUDIO CHAPTERS ARE REAL NOW: AudioPlayer takes chapterOffsets (fractions from audioChapterOffsets() in mock-stories, word-weighted per 4-paragraph block) instead of a hardcoded chapterCount={3}; prev/next actually seek, the label tracks the playhead, and under 3 chapters the whole chapter UI is replaced by a 15s skip. New components/story/StoryGlossaryPanel.tsx: collapsed on mobile, open from md, and the <dl> is always in the DOM (hidden by class) so the definitions stay in the prerendered HTML for SEO. "Créer le livre" print block REMOVED from the public story page, restore instructions in app/_archive/print-prompt/README.md (the copy on histoire-perso was left alone). Breadcrumb pill is rounded-2xl (rounded-full ate the text once the trail wrapped). Sequel CTA wraps instead of nowrap. New "see all <genre> for <age>" button under the 3 related cards. Mobile navbar gained an all-stories BookOpen icon left of the theme toggle (nav.allStories).
2026-08-12 [Session B]: touched components/layout/Header.tsx (A) — new shared components/layout/ChildTopBar.tsx now REPLACES the whole site navbar whenever a child profile is active, so a kid opening a story keeps the bubble bar instead of the parent navigation. Child bar gained the theme toggle and moved the streak to the right cluster. Deleting a child profile no longer orphans their stories: reassignStoriesToParent() moves them to the parent shelf (confirm copy says so).
2026-08-12 [Session A]: MOBILE PASS. Horizontal overflow killed at 320/375/414/768 on every public page (was the navbar cluster at 334px on a 375px bar, the InteractiveBand -m-6 glow, its nowrap CTA setting a grid min-width, and the 280px glossary tooltip). Navbar below lg now shows ONLY the theme toggle + burger; search/language/bell/streak/account moved into the panel, which gained a separate tinted account block (collapsible, shows the account name). Burger + new AccountNavDrawer both lock the body with position:fixed (NOT overflow:hidden, iOS ignores it) and restore scrollY on close. StoryGrid is 1 card per row under sm. DialogContent is w-[calc(100%-2rem)] + max-h-[calc(100dvh-2rem)] so the audio modal fits. New components/account/AccountNavDrawer.tsx replaces the always-visible sidebar under lg. 1-2 story bodies merged 12 -> 8 paragraphs (12 made every sentence its own block). Homepage LatestStories CTA no longer forces ?age=3-4.
2026-08-12 [Session A]: quiz length is now DERIVED, not hand-written: new quizLength(words) in mock-stories.ts (2 under 200 words, 3 under 500, 4 under 900, 5 under 1400, else 6) and storyQuiz() slices to it, so bodies and quizzes can never drift apart. Added the missing questions to 6 older stories. The 1-2 stories were far too short (115-200 words, under 1 min honest read) so all 10 were rewritten to 12 paragraphs / 200-275 words and WORD_RANGE_BY_AGE["1-2"] raised to 200-320; readingMinutes pinned to 2 across the range. Covers generated for the 9 new slugs (pnpm img:generate ONLY=..., manifest now 21). Open filter-category pill was navy-on-navy in dark mode: new --color-catpill-fg token (careful, cream-50 is INVERTED in dark and is NOT a light colour there, ink-800 is). le-renard EN dialogue converted to curly quotes so it picks up the italic styling.
2026-08-11 [Session A]: LIBRARY TAXONOMY TRIMMED + batch 1 of the 60-story plan. Themes 18 -> 12 (dropped aventure/fantastique/humour/decouverte which duplicated genre names, plus voyage/sport), characters 25 -> 23 (dropped gateau/phare/marchand/reine/grand-mere, added groupe-enfants/chien/souris) in lib/stories/filter.ts + mock-stories EXTRA_CHARACTERS; the 12 original stories were RECAST onto it (theme/character fields only, bodies untouched). 9 new 1-2yo stories with FR+EN body/quiz/glossary. New optional titleEn/excerptEn on MockStory + storyTitle()/storyExcerpt() helpers wired into the story page and StoryCard. Fixed "plus récentes" sort (returned array order = OLDEST first; now sorts on publishedAt). renderParagraph italicises EN curly-quote dialogue too (site + histoire-perso). Plan + conventions: app/docs/LIBRARY_EXPANSION_V1.md.
2026-08-11 [Session B]: character wizard preview is TEXT-ONLY for V1 (placeholder box removed); the illustrated avatar is deferred to V2 as a layered SVG, fully specced in V2_CHARACTER_AVATAR.md so it does not need re-deciding. Archetypes are now image cards (/illustrations/archetype-&lt;id&gt;.webp, 3 per row) with prompts in IMAGE_PROMPTS_archetypes.md. Placeholder art everywhere now reveals on LOAD instead of hiding on error (onError never fires when the request failed before the handler attached).
