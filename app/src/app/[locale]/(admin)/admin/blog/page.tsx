import { mockBlogPosts } from "@/data/mock-admin";
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-ink-500)]">
            Articles rédigés avec l'IA puis relus à la main avant publication.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-8 overflow-x-auto rounded-2xl border border-[var(--color-ink-100)] bg-[var(--color-cream-50)]">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[var(--color-ink-100)]/60">
            {mockBlogPosts.map((p) => (
              <tr key={p.id} className="hover:bg-[var(--color-cream-100)]/60">
